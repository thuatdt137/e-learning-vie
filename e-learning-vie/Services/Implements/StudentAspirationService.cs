...
namespace e_learning_vie.Services.Implements
{
    public class StudentAspirationService : IStudentAspirationService
    {
        ...
        public async Task<AspirationDto> CreateAspirationAsync(ClaimsPrincipal user, CreateAspirationDto request)
        {
            ...
            // Tìm class hiện tại của học sinh dựa trên AcademicYear từ ClassHistory
            var currentClass = student.StudentClassHistories
                .FirstOrDefault(sch =>
                    sch.AcademicYear != null &&
                    sch.AcademicYear.StartDate <= DateOnly.FromDateTime(currentDate) &&
                    sch.AcademicYear.EndDate >= DateOnly.FromDateTime(currentDate));
            ...
            if (string.IsNullOrEmpty(className))
                throw new InvalidOperationException("Tên lớp không hợp lệ.");
